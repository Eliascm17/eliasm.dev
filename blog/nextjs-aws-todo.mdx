...
From here the amplify CLI has automatically created mutation, query, and subscription operations based on our previously defined schema. From here we can easily add these into our app to make our simple todo app.
...
Subscriptions are super cool! We can use subscriptions in our app to get live updates to our data so when we create or delete a todo the user will be able to see that in the UI. We can listen to events such as onCreateTodo and onDeleteTodo to listen for when certain mutations happen in our database and update our UI for our user.
